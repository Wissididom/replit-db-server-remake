...
app.get(`/:token/:key`, (req, res, next) => {
  if (
    fs.existsSync(
      path.join(__dirname + `/${req.params.token}/${req.params.key}.txt`),
    )
  ) {
    res.sendFile(
      path.join(__dirname + `/${req.params.token}/${req.params.key}.txt`),
...
app.get(`/:token`, (req, res, next) => {
  if (fs.existsSync(path.join(__dirname + `/${req.params.token}`))) {
    ...
    res.send(
      fs
        .readdirSync(path.join(__dirname + `/${req.params.token}`), {
          withFileTypes: true,
        })
...
app.post(`/:token`, (req, res, next) => {
  ...
  for (let i = 0; i < keys.length; i++) {
    fs.writeFileSync(
      path.join(__dirname + `/${req.params.token}/${keys[i]}.txt`),
      req.body[keys[i]],
    );
